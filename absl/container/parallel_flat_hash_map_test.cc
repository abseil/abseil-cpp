...
INSTANTIATE_TYPED_TEST_SUITE_P(ParallelFlatHashMap, ConstructorTest, MapTypes);
INSTANTIATE_TYPED_TEST_SUITE_P(ParallelFlatHashMap, LookupTest, MapTypes);
INSTANTIATE_TYPED_TEST_SUITE_P(ParallelFlatHashMap, ModifiersTest, MapTypes);

TEST(ParallelFlatHashMap, StandardLayout) {
  struct Int {
  ...
  // copy-constructed.
  {
    parallel_flat_hash_map<Int, Int, Hash> m;
...
TEST(ParallelFlatHashMap, IteratesMsan) {
  // Because SwissTable randomizes on pointer addresses, we keep old tables
  // around to ensure we don't reuse old memory.
  std::vector<absl::parallel_flat_hash_map<int, balast>> garbage;
  for (int i = 0; i < 100; ++i) {
    absl::parallel_flat_hash_map<int, balast> t;
    for (int j = 0; j < 100; ++j) {
      t[j];
...
TEST(ParallelFlatHashMap, BitfieldArgument) {
  union {
    int n : 1;
  };
  n = 0;
  parallel_flat_hash_map<int, int> m;
  m.erase(n);
  m.count(n);
  m.prefetch(n);
  m.find(n);
...
TEST(ParallelFlatHashMap, MergeExtractInsert) {
  // We can't test mutable keys, or non-copyable keys with parallel_flat_hash_map.
  // Test that the nodes have the proper API.
  absl::parallel_flat_hash_map<int, int> m = {{1, 7}, {2, 9}};
...
TEST(ParallelFlatHashMap, Any) {
  absl::parallel_flat_hash_map<int, absl::any> m;
  m.emplace(1, 7);
  auto it = m.find(1);
  ASSERT_NE(it, m.end());
  ...
  absl::parallel_flat_hash_map<absl::any, int, H, E> m2;
  m2.emplace(1, 7);
  auto it2 = m2.find(1);
